...
btnAddCart.forEach((item) => {
    item.addEventListener("click",(e) => {
        ...
        let productName = product.querySelector(".container-fluid.three .card-title").innerText;
        let productPrice = product.querySelector(".container-fluid.three .card-price").innerText;
...
function updateCartTotal() {
    ...
    for (let i = 0; i < cartItem.length; i++) {
        ...
        let productPrice = cartItem[i].querySelector(".price").innerHTML.replace('$','')
...
function displayDate () {
    ...
    document.getElementById('date').innerHTML = "Today is " + day[date.getDay()] + ", "  + months[date.getMonth()] +  " "  +  date.getDate()  + " " +  date.getFullYear();
...
function validateName() {
  ...
  if (contactName.value.length == 0) {
    ...
    errorName.style.color = 'red';
    errorName.innerHTML = "The name must be more than 6 and less than 30 characters long"  ;
    return false;
  }
  else if (contactName.value.length > 6 || contactName.value.length < 30) {
...
function validateEmail () {
  ...
  else if(!contactMail.value.match(/^[A-Za-z\._\-[0-9]*[@][A-Za-z]*[\.][a-z]{2,4}$/)) {
    errorEmail.style.color = 'red';
